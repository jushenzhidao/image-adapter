...
class ObjectStore(Protocol):
    ...
    async def put(self, data: bytes, *, key: str, content_type: str) -> StoredObject:
        ...
        (``temp/<request-id>/<uuid>.<ext>``). A backend without a directory
        concept is expected to map it, not to reject it -- that mapping is a
        large part of why this port exists. ``content_type`` is required
        rather than inferred: callers already sniff the bytes, and a backend
        that stores the type can serve the URL inline instead of as a
        download.

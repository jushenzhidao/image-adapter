...
def build_request(
    ...
    if channel.proxy.covers(url):
        # aiohttp takes the proxy per request, so one channel can leave from a
        # different exit while every other channel stays direct -- no process
        # session change, no second connection pool. The key is set only when

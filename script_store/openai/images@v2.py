"""openai/images@v2: OpenAI-native generations (JSON) and edits (multipart).

v2 in one line: the per-image work that used to run one reference at a time now
runs through ``ctx.fanout``, so N references cost one wait instead of N. Nothing
else changed -- same body, same multipart part order, same error codes, and N=1
still takes the serial path.

That is a *behaviour* change rather than a refactor: a multi-reference edit now
...
back. Hence a new version rather than an edit in place, with ``v1`` left
untouched so a channel pinned to it keeps the old behaviour.

"""volcengine_ark/images@v7: doubao-seedream text-to-image and image-to-image.

Channel setup (New API side):
  X-Upstream-Url: https://ark.cn-beijing.volces.com/api/v3/images/generations
  X-Script-Ref:   volcengine_ark/images@v7
...
What v7 changes: the retry now answers **two** ARK wordings rather than one --
`invalid url specified` joins `Timeout while downloading url=`, so a reference
ARK refused to *parse* is repaired the same way a reference it failed to
*fetch* is. Nothing else moves relative to `@v6`: watermark still defaults to
off, the default `image_ref_mode` is still `url`, and the engine is untouched.
...
The precedence chain is unchanged -- request body, then channel option, then
the default -- so only the last link moved:
...
Why a version rather than an edit of `@v5`: the change is observable with no
option set at all -- a request carrying nothing but a prompt now leaves our side
with `watermark: false` where it used to leave with `watermark: true` -- so any
channel can tell the two apart, and the difference is in the images it serves.
The escape valves are the version pin (`@v5` keeps the old default, `@v2` keeps
it without the retry) and the option itself.

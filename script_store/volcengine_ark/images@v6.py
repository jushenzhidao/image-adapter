"""volcengine_ark/images@v6: doubao-seedream text-to-image and image-to-image.

Channel setup (New API side):
  X-Upstream-Url: https://ark.cn-beijing.volces.com/api/v3/images/generations
  X-Script-Ref:   volcengine_ark/images@v6
...
What v6 changes: **watermark now defaults to off** (the section below). Nothing
else moves relative to `@v5` -- the reference-fetch retry is inherited
unchanged, and the default `image_ref_mode` is still `url`.

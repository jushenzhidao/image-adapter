...
Channel setup (New API side):
  ...
  `model` here is an ARK access point id, not an OpenAI-style model name,
  ...
  fetch that reference ourselves and retry once. See "Answering ARK's own fetch
  timeout" below.
...
This file is the channel's only version. It is the merge of everything this
integration has needed, and the sections below are ordered by the decision each
one makes:

  1. which wire form a reference takes                (`image_ref_mode`)
  2. whether a reference is re-encoded on the way out (`ref_*`)
  3. how those references are materialised            (concurrently, `ctx.fanout`)
  4. what ARK is asked for by default                 (watermark off, stream off)
  5. what happens when ARK cannot fetch a reference   (one retry, inlined)

Provenance, for whoever greps the history: this became `@v1` when the
version-split practice was dropped, and it folds in what used to be separate
numbered releases -- the `ref_*` policy, the retry that answers ARK's fetch
timeout, and the watermark default of off.
...
could go wrong quietly:
  ...
  * **A reference we cannot decode is passed through, not refused.** The
    ...
    request proceeds unchanged. Decode refusals only, though: a reference we
    failed to *fetch* still fails the request, because that is a fact about the
    URL rather than about the optimisation, and retrying it here would spend the
    same timeout twice.
...
one wait instead of N -- the same primitive the other channels' scripts use
(docs/07 §14.2.2).

What a reference costs is dominated by the waits around it: fetching a URL,
uploading a re-hosted one. Those are what the fan-out hides. The *re-encode*
inside each item still runs one at a time, because ``binascii`` and Pillow hold
the GIL and overlapping CPU work would add concurrency without adding
throughput. In the default `url` mode with no `ref_*` set, N references are N
uploads and no downloads; in `data_uri` mode they are N downloads. Both are
waits.
...
the serial arm, and the failure semantics are a serial loop's -- the earliest
...
puts up to `fanout_concurrency` requests on that caller's origin at once. The
body is unaffected; the timing is not.
...
Nothing has to be configured for that; a channel that would rather never see
that first failure can set `image_ref_mode: "data_uri"` and pay a local download
on every reference instead.
...
def _ref_policy(ctx):
    ...
    None is the important return value: it keeps a channel that never opted in
    on the shortest path -- the same three conversions the policy branch below
    performs, minus the redundant decode it would repeat -- so an unused option
    group cannot change what the request looks like.
    """
...
async def _compressed_or_kept(ctx, ref, policy):
    ...
    It proceeds with its original bytes -- an optimisation must not turn a
...
async def _to_ark_ref(ctx, ref):
    ...
    `image_ref_mode` decides the form for every shape, a URL included. An
    earlier implementation returned URLs before the mode was read at all, so
    ...
    if any(reason in message for reason in REFERENCE_FAILURES):
        ...
        mode = "data_uri"
    if mode == "inline":
        # The name the google script uses for this same behaviour. Accepting it
    ...
    if policy is None:
        # No policy: the shortest path, kept verbatim so "the channel did not
        # opt in" is provably the same request as before the option existed.
        if mode == "base64":
...
async def transform(ctx, payload, phase):
    if phase == "request":
        body = {
            ...
            "model": ctx.mapped_model
            or ctx.options.get("model", "doubao-seedream-5-0-260128"),

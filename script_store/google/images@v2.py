"""google/images@v2: Gemini Image ("Nano Banana") through generateContent.

v2 in one line: the two per-image loops -- fetching the client's references and
re-hosting the reply's images -- now run through ``ctx.fanout``, so N images cost
one wait instead of N. Nothing else changed: same parts, same order, same error
codes, and N=1 still takes the serial path.

Two places had to be handled differently from ``openai/images@v2``:
...
* **The reference fetches are hoisted, not parallelised in place.** Whether a
  ...
  *can* run concurrently is the download itself, so the downloads are pulled out
  ...
  as before. The set of references prefetched is exactly the set the old loop
  would have fetched -- no speculative work, and no wasted download if a later
...
That the concurrency is a *behaviour* change (up to ``fanout_concurrency``
concurrent requests on the caller's image host, and on the object store) is why
this is a new version and ``v1`` is left untouched.

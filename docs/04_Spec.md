...
| AC-34 | 模型映射 | When 渠道声明 `X-Model-Map`，系统必须按「精确命中 > `*` 兜底 > 不改」解析客户端 `model`，命中时改写 canonical body 的 `model`（四个入口一致，且客户端可见回显仍是客户端所请求的名字），并把解析结果以 `ctx.mapped_model` 交给脚本；未声明该头时不得改动请求中任何字段；表为「精确条目 + **至多一条** `*` 兜底」，重复键（含去掉空白后重复）必须拒绝而非静默取末值；表不可用（配对不对缺 `=`、键或模型为空、含 `*` 的非裸通配键）必须在任何上游调用之前报 `channel_config_error`(400)；已废弃的 `X-Channel-Options.model_map` 键必须同样报错而非静默忽略 | P0 |
| AC-35 | 尺寸方言归一化 | When 脚本调用 `ctx.parse_size(raw, ratios=…, tiers=…, resolutions=…)`，系统必须**只依据调用方给的厂商事实**判定方言（比例 / 绝对分辨率 / 档位 / 未设置 / 读不懂，五类）：比例给出其规范分辨率；等于某比例规范值的分辨率（`*` 与 `x` 两种分隔符、大小写不敏感）必须归一到该比例并留下 note；非规范分辨率保留原值且 `exact=True`；档位匹配大小写不敏感且**回厂商表里的拼法**，且不得被误读为分辨率；`resolutions=False` 时绝对分辨率一律 `unknown`；**未提供事实时必须判 `unknown`，不得凭空造值** | P1 |
| AC-36 | 内容审核拒答的出口契约 | When 任一渠道脚本判定上游因**内容安全策略**拒绝（厂商专属 code 命中，或自然语言词命中跨渠道共享词表），系统必须经 `ctx.fail_moderation(...)` 返回 **400**（`code=content_filter`、`err_type=invalid_request_error`、`param=prompt`），消息含厂商原文且以统一后缀说明不可重试；**不得**落进 5xx（5xx 会被下游重试，而同一 prompt 必再被拒）。判定必须同时认「调用方传入的厂商 code」与「`capabilities/_shared.json` 的 `moderation_wordings`」；**共享表缺失时退化为内置实测词表，不得退化为沉默**（沉默＝把拒答当成可重试的上游抖动）。命中必须留 trace note（`outcome=content_refused` + 厂商事实如 modality/stage） | P0 |
...
| 2026-09-11 | `/v1/images/edits` 的**空 `image` part 改判为文生图**（整键删除，不再 400） | multipart 分不清「字段没传」与「字段传了空值」：HTML 未选文件、SDK 读到空文件都提交 0 字节 part，此前一律 `InvalidRequestError: 'image' must not be empty`，把调用方本意的文生图请求打回 400。判定按载体而非放宽校验：JSON／chat／responses 能表达该区别，`image: ""` 在那里仍是 400 | `adapter/api/image_edits.py`（`_read_upload` 空件返 `None` + 空字段整键省略，`mask` 同规则）；`tests/integration/test_image_edits_endpoint.py`（1 删 5 增）；README 契约表与差异说明 |
| 2026-09-11 | **非法 `response_format` 改判为「未指定」并删除该键**（不再 400）；同时修掉不可哈希值（list/object）触发的 `TypeError`→500 | `validate_images_body` 是唯一把它当致命错误的地方，而三个脚本早就按「非法＝沉默」读（`openai/images@v1::_requested_format` 把集合外的值映射成 `None`，`google` 亦然）⇒ 门面比真正使用该字段的组件更严。`null`／空串／chat 形态 `{"type":…}` 同走此路；键删除而非置空，理由与 `image` 相同（`openai/images@v1` 文生图路径整体转发）。不承诺被打破：适配器本来就只按能产出的形态作答（AC-03/AC-04） | `adapter/api/images.py`；`tests/unit/test_images_validation.py`（1 改 3 增）、`tests/integration/{test_images_endpoint,test_async_polling,test_image_edits_endpoint}.py`（各 1 改，后两者另增 1）；README:7 与输出形态表、`docs/09` §2.1 |
| 2026-09-19 | **AC-36 落地**：`ModerationMixin`（判定 + 出口契约一处）+ `capabilities/_shared.json`（跨渠道共享词表）；qwen 脚本改用它并**删掉脚本里的私有词表** | 同一件事在 qwen/google 各写一份（且 **ark 一份都没有** ⇒ 同类 bug 潜伏）；三个脚本是单文件、沙箱只给 stdlib ⇒ **无法共享代码**，所以「一处」只能落在框架侧。厂商 code 仍由调用方传入（共享 code 表才是猜） | `adapter/ctxapi/moderation.py`(新)、`adapter/capabilities.py`、`script_store/capabilities/_shared.json`(新)、`script_store/qwen/images@v1.py`、`tests/unit/test_ctx_moderation.py`(新 +7) |
| 2026-09-18 | **AC-35 落地**：`ctx.parse_size` + `SizeReading`（`adapter/ctxapi/mapping.py`）；qwen 脚本改为调用它并删掉脚本内的重复实现 | 三种尺寸方言的识别与归一化在**每个**图片上游都要写一遍，而写错的失败是静默的（请求照样成功，只是尺寸与账单不同）；出线形态与替换策略仍归脚本，所以只抽"读法"不抽"决定"（`ctx.map_size` 仍不做，理由见 `docs/07` §5.1） | `adapter/ctxapi/mapping.py`、`adapter/ctxapi/__init__.py`、`script_store/qwen/images@v1.py`、`tests/unit/test_mapping.py`(+8)、`tests/unit/test_ctx_composition.py`(1 改) |
| 2026-09-19 | **`mask` 在火山方舟渠道被静默丢弃 —— 裁决「维持现状 + 文档标注」**（不转发、不报错、**不改脚本**）：实测客户端带 `image`+`mask` 得 200，假上游回读**上游侧 body** 不含 `mask`（脚本对该字段零引用）⇒ inpaint 静默退化为普通图生图，且出图照常计费 | 三选项（转发 / 不支持则显式 400 / 维持并标注）中用户选第三项：转发前需先确认 ARK 是否接受该字段（未验），而让存量请求开始报错的代价大于收益；但**必须让调用方看得见**，否则就是"成功的静默错误" | `README.md`（契约段新增警示）、`docs/05_Volcengine_ARK_Integration.md`（§已知限制新增第 5 条）；**代码零改动**（脚本内容与摘要均未变）；证据 `reports/2026-09-19_ark-forms-concurrent/`（`probe_mask_zero_cost.py` + `meta/mask_zero_cost.json`） |

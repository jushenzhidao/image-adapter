...
def test_a_data_uri_upstream_becomes_a_link_when_storage_is_configured(
    ...
    assert raw == PNG_1X1
    # <yyyymmdd>/<uuid>.<ext>: the day leads so a bucket lifecycle rule can expire a day
    # at a time, and there is no prefix unless a deployment asks for one. Neither the model
    # nor the request id is in the key -- a key becomes a URL a caller can hold.
    day, name = key.split("/")

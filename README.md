...
⚠️ **`mask` 是门面接受的字段，但不保证被转发给上游** —— 它会被规范化后交给脚本，**送不送由该渠道的脚本决定**。例如火山方舟渠道（`volcengine_ark/images@v1`）对 `mask` **零引用**：客户端带 `mask` 仍得 200，而**上游实收 body 里没有该字段**（2026-09-19 假上游回读实测）⇒ **inpaint 静默退化成普通图生图**，没有任何信号。当前处置是「维持现状 + 文档标注」（见 `docs/05` §已知限制 5）。接入需要 mask 语义的渠道前，请先确认该脚本是否转发 `mask`。
...
| `X-Upstream-Proxy` | 否 | 本渠道单独走出的 HTTP 代理，如 `http://127.0.0.1:3128`。覆盖**除素材下载外**的所有出站（引擎代发 + 脚本经 `ctx.http` 的调用）；`UPSTREAM_PROXY_BYPASS_HOSTS` 里的 host 除外（把它当"出口够不着"的名单：内网/回环服务、或任何不该走付费出口的目标）。**空＝任意 host**（`UPSTREAM_PROXY_ALLOWLIST` 是**限制**不是开关：设了值则只放行清单内的 host，off-list 仍在调上游之前 400）。**不支持 SOCKS**（`socks5://` 被明确拒绝），需在适配器旁跑一个 SOCKS→HTTP 桥。**部署级默认**见 `UPSTREAM_PROXY_DEFAULT`（头缺省时生效、头有值则头赢；裸值 `none` = 该渠道显式直连，免受默认代理影响） |
...
契约其实有三份（解析器的 `H_*` 常量、FastAPI 声明、CORS 允许清单），靠人工对齐迟早会漂：
...
> `model`（`relay/helper/model_mapped.go`）。两者同时配置是**串联**的：new-api 先改写，本表再按
> **改写后**的名字匹配。所以本表的键必须写 new-api 映射**之后**的名字；写客户端原名则永不命中
> （退化为透传，无害），而写 `"*"` 会反过来**盖掉** new-api 的决定（此时控制面的路由/计费陈述
> 失真，且没有任何报错）。
>
> 判据：**new-api 的 `model_mapping` 能用精确键表达时，不必在这里再配一遍**。本表不可替代的只有两点——
> `"*"` 兜底（new-api 的映射是精确键，没有通配），以及绕过 new-api 的直连调用方（探针、`/docs` 试调）。

...
> **渠道选项一律按渠道携带**：没有部署级默认选项（`DEFAULT_CHANNEL_OPTIONS` 已于 2026-09-22 移除
> ——数据面上不共享任何运营者身份；渠道带自己的选项，qwen 的 pair 形态带自己登录出来的 jar）。
...
回环/私网地址在这里是**允许**的——把代理放在本机（loopback 上的网关/转换器）正是它的用法。
